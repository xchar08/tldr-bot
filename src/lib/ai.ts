...
export class AiService {
  ...
  constructor() {
    // 1. Force the client to use ONLY our key and URL
    this.client = new OpenAI({
      apiKey: config.NEBIUS_API_KEY,
      baseURL: 'https://api.studio.nebius.ai/v1/',
      // 2. Disable default header loading from env just in case
      dangerouslyAllowBrowser: true, // Sometimes helps in weird node contexts, though unrelated to 401
    });
  }
  ...
  async summarizeMessages(messages: string[], maxTokens = 250): Promise<string> {
    ...
    try {
      ...
      // 4. FALLBACK: Try raw fetch if OpenAI client fails (isolates library issues)
      if (error.status === 401) {
  ...
  // Raw fetch implementation to bypass OpenAI SDK issues
